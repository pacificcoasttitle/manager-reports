...
async function buildRepEmailHtml(repName) {
  ...
  const [{ rows: mtd }, { rows: opens }, { rows: yday }, { rows: ydayOpen }, { rows: prior }, { rows: wd }] = await Promise.all([
    pool.query(`
      SELECT COUNT(*) as cnt,
        ...
        ROUND(COALESCE(SUM(CASE WHEN order_type IN ('Title & Escrow','Escrow Only') THEN commissionable_escrow ELSE 0 END),0)::numeric, 2) as comm_escrow
    ...
    pool.query(`
      SELECT ROUND(COALESCE(
               ...
               + SUM(CASE WHEN order_type IN ('Title & Escrow','Escrow Only') THEN commissionable_escrow ELSE 0 END)
             ,0)::numeric, 2) as rev
      FROM order_summary WHERE sales_rep = $1 AND fetch_month = $2
    `, [repName, priorMonth]),
  ...
  const pctChange = priorRev > 0 ? Math.round(((repTotal - priorRev) / priorRev) * 100) : null;
