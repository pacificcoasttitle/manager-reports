 ...
 *
 * SCAFFOLDING PHASE: revenue = commissionable_escrow (the REP base) scoped by
 * escrow_revenue > 0 (the cured definition, matches the reconciliation bar).
 * After Brandon rules on the wider officer base (+ loan tie-in + docs), swap
 * commissionable_escrow -> officer_commissionable_escrow in the queries below.
 *
 * Recipients live in officer_email_recipients (officer_type = 'escrow').
 * NOT wired into the cron — test sends only this phase.
 */
...
async function buildEscrowOfficerEmailHtml(officerName) {
  ...
  // SCAFFOLDING: commissionable_escrow (rep base), scoped by escrow_revenue > 0
  const [{ rows: mtdClose }, { rows: mtdOpen }, { rows: ydayClose }, { rows: ydayOpen }, { rows: priorClose }, { rows: workDays }] = await Promise.all([
    pool.query(`
      SELECT COUNT(*) FILTER (WHERE commissionable_escrow > 0) as total_cnt,
        ROUND(COALESCE(SUM(commissionable_escrow),0)::numeric, 2) as revenue
      FROM order_summary
    ...
    pool.query(`
      SELECT ROUND(COALESCE(SUM(commissionable_escrow),0)::numeric, 2) as revenue
      FROM order_summary
      WHERE escrow_officer = $1 AND fetch_month = $2 AND escrow_revenue > 0
  ...
  const { rows: closingsList } = await pool.query(`
    ...
    WHERE escrow_officer = $1 AND fetch_month = $2
      AND escrow_revenue > 0 AND commissionable_escrow > 0
...
// NOT wired into cron yet — defined for when Brandon rules + officer field lands.
...
// Sends every active escrow officer's email to a SINGLE test address with a TEST/SCAFFOLDING banner.
async function sendEscrowOfficerEmailsTest(testEmail) {
  ...
  for (const officer of officers) {
    try {
      const { html, subject } = await buildEscrowOfficerEmailHtml(officer.officer_name);
      const banner = `<div style="background:#fef3c7; color:#92400e; padding:12px 20px; text-align:center; font-family:Arial,sans-serif; font-size:13px; font-weight:600;">\u26a0\ufe0f TEST \u2192 ${officer.officer_name} at ${officer.email} \u00b7 SCAFFOLDING (rep-base number, not final officer base)</div>`;
      await sgMail.send({
        to: testEmail,
        from,
