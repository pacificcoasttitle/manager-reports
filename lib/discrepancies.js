...
async function runDiscrepancyChecks(month, year) {
  ...
  // 5. Orders where title officer is not in mapping table (branch = Unassigned)
  // This overlaps with check #13 but focuses on ALL orders, not just those with a title officer
  try {
    const { rows } = await pool.query(`
      SELECT os.file_number, os.title_officer, os.category, os.sales_rep, 
        os.total_revenue, os.transaction_date::text
      FROM order_summary os
      LEFT JOIN title_officer_branches tob ON os.title_officer = tob.officer_name
      WHERE os.fetch_month = $1 
        AND (os.title_officer IS NULL OR os.title_officer = '' OR tob.officer_name IS NULL)
    ...
    if (rows.length > 0) {
      checks.push({
        ...
        severity: 'warning',
        title: 'Orders with Unassigned Branch',
        description: `${rows.length} orders have no title officer or their title officer is not in the branch mapping table. These appear under "Unassigned" in reports.`,
        count: rows.length,
        details: rows.slice(0, 20),
        columns: ['file_number', 'title_officer', 'category', 'sales_rep', 'total_revenue', 'transaction_date']
      });
    }
  } catch (e) { console.error('Check unassigned-branch failed:', e.message); }
  ...
  // 9. Branch revenue dropped >30% from prior month (branch from title officer mapping)
  try {
    const { rows } = await pool.query(`
      WITH current AS (
        SELECT COALESCE(tob.branch, 'Unassigned') as branch, ROUND(SUM(os.total_revenue)::numeric, 2) as rev
        FROM order_summary os
        LEFT JOIN title_officer_branches tob ON os.title_officer = tob.officer_name
        WHERE os.fetch_month = $1 
        GROUP BY COALESCE(tob.branch, 'Unassigned')
      ),
      prior AS (
        SELECT COALESCE(tob.branch, 'Unassigned') as branch, ROUND(SUM(os.total_revenue)::numeric, 2) as rev
        FROM order_summary os
        LEFT JOIN title_officer_branches tob ON os.title_officer = tob.officer_name
        WHERE os.fetch_month = $2
        GROUP BY COALESCE(tob.branch, 'Unassigned')
      )
      SELECT c.branch,
  ...
  // 13. Title officers in data but not in mapping table
  try {
    const { rows } = await pool.query(`
      SELECT os.title_officer, COUNT(*) as order_count,
        ROUND(SUM(os.total_revenue)::numeric, 2) as total_revenue
      FROM order_summary os
      LEFT JOIN title_officer_branches tob ON os.title_officer = tob.officer_name
      WHERE os.fetch_month = $1
        AND os.title_officer IS NOT NULL AND os.title_officer != ''
        AND tob.officer_name IS NULL
      GROUP BY os.title_officer
      ORDER BY total_revenue DESC
    `, [yearMonth]);

    if (rows.length > 0) {
      checks.push({
        id: 'unmapped-title-officer',
        severity: 'critical',
        title: 'Title Officers Not in Branch Mapping',
        description: `${rows.length} title officers appear in orders but are not in the branch mapping table. Their revenue is missing from the Title Officer report.`,
        count: rows.length,
        details: rows,
        columns: ['title_officer', 'order_count', 'total_revenue']
      });
    }
  } catch (e) { console.error('Check unmapped-title-officer failed:', e.message); }

  // 14. Open orders with a type that matches no report's Openings filter
  try {
    const { rows } = await pool.query(`
      SELECT order_type, COUNT(*)::int as count
      FROM open_orders
      WHERE open_month = $1
        AND (order_type IS NULL OR LOWER(order_type) NOT IN ('title only', 'title & escrow', 'escrow only', 'trustee sale guarantee'))
    ...
    if (total > 0) {
      checks.push({
        id: 'orphan-open-orders',
        severity: 'warning',
        title: 'Open Orders With Unrecognized Type',
        description: `${total} open orders have an order_type that does not match any report's Openings filter, so they appear on no Openings sub-tab.`,
        count: total,
        details: rows,
        columns: ['order_type', 'count']
      });
    }
  } catch (e) { console.error('Check orphan-open-orders failed:', e.message); }
