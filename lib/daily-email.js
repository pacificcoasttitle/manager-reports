...
async function sendDailyReport(overrideDate) {
  ...
  const { rows: settingRows } = await pool.query(
    "SELECT key, value FROM app_settings WHERE key IN ('daily_email_enabled', 'daily_report_recipients')"
  );
  const settings = {};
  ...
  // Get recipients: settings table first, fall back to env var
  const recipientStr = settings.daily_report_recipients || process.env.DAILY_REPORT_RECIPIENTS || '';
