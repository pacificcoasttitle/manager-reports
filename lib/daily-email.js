 ...
 *   key: 'daily_email_recipients'   value: 'ceo@pct.com,gm@pct.com'
...
// Per-working-day pace comparison rendered as a short table cell (▲/▼ X%).
// Delegates the math to the shared pctChangePace helper so it can't drift from
// the other emails; just reformats to the compact cell style used in the grid.
function pctChange(currentMTD, priorFullMonth, workedDays, priorTotalWorkingDays) {
  const t = pctChangePace(currentMTD, priorFullMonth, workedDays, priorTotalWorkingDays);
  if (t.pct === null) return { text: '—', color: '#868e96' };
  return {
    text: (t.pct >= 0 ? '▲ ' : '▼ ') + Math.abs(t.pct) + '%',
    color: t.color
  };
}
...
async function buildDailyReportHtml(overrideDate) {
  ...
  console.log(`[daily-email] yesterday=${yesterdayStr}  yearMonth=${yearMonth}  priorMonth=${priorMonth}`);
  ...
  const [
    ...
    { rows: topRepsRows },
    { rows: workDaysRows },
    { rows: escrowFileBranchRows },
    { rows: priorWorkDaysRows }
  ] = await Promise.all([
    pool.query(`
      SELECT title_officer, category, COUNT(*) as cnt,
             ROUND(SUM(total_revenue)::numeric,2) as revenue
      FROM order_summary WHERE transaction_date::date = $1::date
      GROUP BY title_officer, category
    `, [yesterdayStr]),

    pool.query(`
      SELECT title_officer, category, COUNT(*) as cnt
      FROM open_orders WHERE received_date::date = $1::date
      GROUP BY title_officer, category
    `, [yesterdayStr]),
    ...
    pool.query(`
      ...
      FROM order_summary
      WHERE fetch_month = $1
        AND (file_number LIKE '%-ONT' OR file_number LIKE '%-PRV')
      GROUP BY file_branch
      ORDER BY file_branch
    `, [yearMonth]),
    ...
    pool.query(`
      ...
      FROM generate_series(
        ($1 || '-01')::date,
        (date_trunc('month', ($1 || '-01')::date) + interval '1 month - 1 day')::date,
        '1 day'
      ) d
      WHERE EXTRACT(DOW FROM d) NOT IN (0, 6)
    `, [priorMonth])
  ...
  const priorTotalWorkingDays = parseInt(priorWorkDaysRows[0]?.total || 0);
  ...
  // Escrow footnote: IE + Porterville revenue from file-number branches
  let escrowFootnoteRevenue = 0;
  const escrowFootnoteParts = [];
  escrowFileBranchRows.forEach(r => {
    const rev = parseFloat(r.revenue) || 0;
    escrowFootnoteRevenue += rev;
    escrowFootnoteParts.push(`${r.file_branch}: ${fmt(rev)}`);
  });
  ...
  <tr>
    <td style="padding:20px 32px 0;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="font-size:12px; border-collapse:collapse;">
        ...
        <tr>
          <td style="padding:6px 8px; color:#868e96; border-bottom:1px solid #f1f3f5; font-size:11px;">vs Last Month's Pace</td>
          ${BRANCHES.map(b => {
            const c = pctChange(mtd[b].revenue, prior[b], workedDays, priorTotalWorkingDays);
          ...
          ${(() => {
            const c = pctChange(totals.mtdRevenue, totals.priorRevenue, workedDays, priorTotalWorkingDays);
      ...
      ${escrowFootnoteRevenue > 0 ? `
      <div style="font-size:11px; color:#868e96; margin-top:8px; font-style:italic;">
        Escrow business not shown above (${escrowFootnoteParts.join(', ')}): ${fmt(escrowFootnoteRevenue)} MTD
      </div>` : ''}
    </td>
  </tr>
