...
async function buildManagerEmailHtml(managerName) {
  ...
  const [{ rows: wd }, { rows: teamRows }, { rows: priorWorkDays }] = await Promise.all([
    pool.query(`
  ...
  const teamTrend = pctChangePace(teamMtdRev, teamPriorRev, worked, priorTotalWorkingDays);
  const teamPctChange = teamTrend.pct;
  ...
  const trendHtml = `<span style="color:${teamTrend.color};">${teamTrend.text}</span>`;
