...
function aggregateLineItems(lineItems) {
  ...
  for (const item of lineItems) {
    ...
    // Commissionable escrow: allowlisted ESC descriptions only (settlement + credits).
    // Standalone subset of escrow_revenue — NOT part of total_revenue. Dormant in Phase 1.
    if (revenueType === 'escrow') {
      ...
      if (desc.includes('settlement') || desc.includes('courtesy') ||
        ...
        order.commissionable_escrow += amount;
      }
    }
