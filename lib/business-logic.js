...
function aggregateLineItems(lineItems) {
  ...
  for (const item of lineItems) {
    ...
    if (!orders[fileNumber]) {
      orders[fileNumber] = {
        ...
        total_revenue: 0,
        commissionable_escrow: 0,
        officer_commissionable_escrow: 0,
        _commPositive: 0,
        _officerCommPositive: 0,
        _escrowNegatives: 0,
        transaction_date: item.transaction_date,
        received_date: item.received_date,
        disbursement_date: item.disbursement_date,
        escrow_closed_date: item.escrow_closed_date,
        marketing_source: item.marketing_source || null,
    ...
    // Keep the first non-empty address seen for this order
    if (!orders[fileNumber].full_address && item.full_address) {
      orders[fileNumber].full_address = item.full_address;
    }
    ...
    // Commissionable escrow: positive allowlisted settlement/credit lines, NET of ALL
    // negative ESC lines (reversals/credits/refunds). Standalone subset of escrow_revenue —
    // NOT part of total_revenue. Finalized + capped after the loop (see below).
    if (revenueType === 'escrow') {
      if (amount < 0) {
        order._escrowNegatives += amount;
      } else {
        const desc = (item.charge_description || '').toLowerCase();
        // Rep base (UNCHANGED): settlement + credits
        if (desc.includes('settlement') || desc.includes('courtesy') ||
            desc.includes('credit') || desc.includes('discount')) {
          order._commPositive += amount;
        }
        // Officer base (wider): rep keywords + loan tie-in + docs
        if (desc.includes('settlement') || desc.includes('courtesy') ||
            desc.includes('credit') || desc.includes('discount') ||
            desc.includes('loan tie') || desc.includes('tie in') ||
            desc.includes('doc')) {
          order._officerCommPositive += amount;
        }
      }
    }
  ...
  // Finalize commissionable escrow: net the negatives, cap at full escrow (structural
  // guard so commissionable can never exceed full), floor at 0.
  for (const order of Object.values(orders)) {
    let comm = order._commPositive + order._escrowNegatives;
    comm = Math.min(comm, order.escrow_revenue);
    comm = Math.max(comm, 0);
    order.commissionable_escrow = Math.round(comm * 100) / 100;

    // Wider officer base: same negatives, wider positive allowlist, same cap/floor.
    let oComm = order._officerCommPositive + order._escrowNegatives;
    oComm = Math.min(oComm, order.escrow_revenue);
    oComm = Math.max(oComm, 0);
    order.officer_commissionable_escrow = Math.round(oComm * 100) / 100;

    delete order._commPositive;
    delete order._officerCommPositive;
    delete order._escrowNegatives;
  }
