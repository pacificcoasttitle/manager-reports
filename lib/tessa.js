...
BRANCHES:
- Branch is determined by the TITLE OFFICER's home branch, using the title_officer_branches mapping table.
...
- If an order has no title officer or the title officer is not in the mapping table, it goes to "Unassigned"
- The stored "branch" column in order_summary (from file number suffix) is NOT used in reports — only the title officer mapping determines branch.
- The file number suffix (GLT, OCT, ONT, PRV) is still stored but is legacy data. For SQL queries involving branch, JOIN with title_officer_branches on title_officer = officer_name.
...
REPORT SPECIFICS:
- Daily Revenue: Shows Purchase, Refi, and TSG by branch (Escrow excluded — has its own report). Includes both open and closed counts plus revenue.
...
- Title Officer Production: Only Purchase and Refinance orders. Grouped by officer's home branch then title officer. Shows only title_revenue + underwriter_revenue.
