...
REPORT SPECIFICS:
- Daily Revenue (Title Revenue tab): Any closed order with (title_revenue + underwriter_revenue) > 0. Revenue shown = that sum (escrow and TSG fees are excluded — they each have their own tab). Purchase/Refi columns only; Title & Escrow files appear in Purchase or Refinance by trans_type. Branch = title officer's home branch. Open counts cover Title only + Title & Escrow order types; TSG opens and Escrow-only opens are excluded here (tracked on their own tabs).
...
- Title Officer Production: Any closed order where (title_revenue + underwriter_revenue) > 0 — includes Purchase, Refinance, and Title & Escrow (category may still be "Escrow" for T&E; reports filter by revenue, not category alone). Revenue shown = title_revenue + underwriter_revenue only. Branch = title officer's home branch. Title & Escrow orders also appear in Escrow Production for the escrow portion.
- Escrow Production (By Sales Rep): Any closed order where escrow_revenue > 0 — includes Title & Escrow and Escrow-only files. Revenue shown = escrow_revenue only (not total_revenue). Branch = file number suffix. Grouped by branch then sales rep. Same Title & Escrow order can appear in both Title Officer and Escrow Production with different revenue columns.
- Escrow Production (By Escrow Officer): Same data and total as the Sales Rep view, just grouped by escrow_officer instead. Use to see who is actually handling the escrow work. Both sub-tab totals must equal each other.
- TSG Production: Only "Trustee Sale Guarantee" orders. Grouped by branch then sales rep.

CLOSINGS vs OPENINGS SUB-TABS:
- Every production report (Title Revenue, Title Officer, Escrow Production by Sales Rep, Escrow Production by Escrow Officer, TSG Production, R-14 Branches, R-14 Ranking) has two sub-tabs: Closings (default) and Openings.
- Closings view: orders that CLOSED with revenue (uses transaction_date). Has Today/MTD/Prior counts AND revenue columns, plus the closing-ratio columns.
- Openings view: orders that were RECEIVED/opened (uses received_date for "today", open_month for MTD/prior). Has Today/MTD/Prior COUNT columns only — NO revenue (revenue is recognized at close, not at open). Same closing-ratio columns.
- Each report's Openings view uses the same population filter as its Closings view: Title reports = Title only + Title & Escrow; Escrow reports = Title & Escrow + Escrow Only; TSG = Trustee Sale Guarantee; R-14 = all order types.
- All Openings queries exclude test data (file_number not like test%/ar test%, profile not '%test & training%').
- Closings and Openings totals are INDEPENDENT populations and do not reconcile to each other; the closing ratio relates them. The reconciliation bar reflects Closings (revenue) only and is hidden on Openings views.
