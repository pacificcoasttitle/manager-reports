...
REPORT SPECIFICS:
- Daily Revenue (Title Revenue tab): Any closed order with (title_revenue + underwriter_revenue + tsg_revenue) > 0. Revenue shown = that sum (not total_revenue, so escrow fees are excluded). Purchase/Refi/TSG columns; Title & Escrow files appear in Purchase or Refinance by trans_type. Branch = title officer's home branch. Open counts use order types that drive those columns (Title only, Title & Escrow, Trustee Sale Guarantee). Escrow-only opens are excluded here.
