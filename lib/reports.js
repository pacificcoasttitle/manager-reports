...
async function r14Branches(month, year) {
  ...
  // Closing ratio: need 4-month window
  const closingMonths = [];
  for (let i = 3; i >= 0; i--) {
    const d = new Date(year, month - 1 - i, 1);
    closingMonths.push(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`);
  }
  // Created = orders received in the window (regardless of when they closed)
  ...
  // Closed = orders with transaction_date in the window (regardless of when received)
...
async function r14Ranking(month, year) {
  ...
  // Closing ratio: two separate queries for created vs closed populations
  // Created = orders received in the 4-month window (regardless of when they closed)
  const { rows: createdInWindow } = await pool.query(
    `SELECT sales_rep, COUNT(*) as cnt FROM order_summary 
     WHERE received_date >= $1 AND received_date <= $2
     GROUP BY sales_rep`,
  ...
  // Closed = orders with transaction_date in the 4-month window (regardless of when received)
  const { rows: closedInWindow } = await pool.query(
    `SELECT sales_rep, COUNT(*) as cnt FROM order_summary 
     WHERE transaction_date >= $1 AND transaction_date <= $2
     GROUP BY sales_rep`,
...
async function titleOfficerProduction(month, year) {
  ...
  // Created = orders received in the window (Purchase & Refinance only)
  const { rows: createdInWindow } = await pool.query(
  ...
  // Closed = orders with transaction_date in the window (Purchase & Refinance only)
  const { rows: closedInWindow } = await pool.query(
    `SELECT title_officer, COUNT(*) as cnt FROM order_summary 
     WHERE transaction_date >= $1 AND transaction_date <= $2
     AND category IN ('Purchase', 'Refinance')
...
async function escrowProduction(month, year) {
  ...
  // Created = escrow orders received in the window
  const { rows: createdInWindow } = await pool.query(
    `SELECT sales_rep, COUNT(*) as cnt FROM order_summary 
     WHERE received_date >= $1 AND received_date <= $2
     AND category = 'Escrow'
  ...
  // Closed = escrow orders with transaction_date in the window
  const { rows: closedInWindow } = await pool.query(
    `SELECT sales_rep, COUNT(*) as cnt FROM order_summary 
     WHERE transaction_date >= $1 AND transaction_date <= $2
     AND category = 'Escrow'
