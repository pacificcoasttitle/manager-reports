...
// ============================================
// TRENDS DASHBOARD (month-over-month time series)
// ============================================
// Revenue + closed-order counts from order_summary (by fetch_month),
// open-order counts from open_orders (by open_month). Optional product-type
// filter (category) and optional series breakdown (branch | product).
// Revenue uses total_revenue so the most recent month matches the reconciliation
// grand total. Purely additive — touches no existing report.

const TRENDS_BRANCH_CASE = `
  CASE
...
function trendsSeriesExpr(breakdown) {
  if (breakdown === 'branch') return TRENDS_BRANCH_CASE;
  if (breakdown === 'product') return 'category';
  return `'Total'`;
}
...
function pivotTrendRows(rows) {
  ...
  for (const r of rows) {
    if (!monthMap[r.fetch_month]) monthMap[r.fetch_month] = { month: r.fetch_month };
    monthMap[r.fetch_month][r.series] = parseFloat(r.value);
...
async function trends({ product = 'all', breakdown = 'none' } = {}) {
  const filtered = product && product !== 'all';
  const seriesExpr = trendsSeriesExpr(breakdown);
  const groupExtra = breakdown !== 'none' ? ', series' : '';
  const params = filtered ? [product] : [];
  const productFilter = filtered ? 'AND category = $1' : '';

  const revenueQuery = `
    SELECT fetch_month, ${seriesExpr} as series, ROUND(SUM(total_revenue)::numeric, 2) as value
    FROM order_summary
    WHERE file_number NOT ILIKE 'test%' AND file_number NOT ILIKE 'ar test%'
      ${productFilter}
    GROUP BY fetch_month${groupExtra}
    ORDER BY fetch_month`;

  const closedQuery = `
    SELECT fetch_month, ${seriesExpr} as series, COUNT(*) as value
    FROM order_summary
    WHERE file_number NOT ILIKE 'test%' AND file_number NOT ILIKE 'ar test%'
      ${productFilter}
    GROUP BY fetch_month${groupExtra}
  ...
  const openQuery = `
    SELECT open_month as fetch_month, ${seriesExpr} as series, COUNT(*) as value
    FROM open_orders
    WHERE file_number NOT ILIKE 'test%' AND file_number NOT ILIKE 'ar test%'
      AND (profile NOT ILIKE '%test & training%' OR profile IS NULL)
      ${productFilter}
    GROUP BY open_month${groupExtra}
    ORDER BY open_month`;

  const [{ rows: revenueRows }, { rows: closedRows }, { rows: openRows }] = await Promise.all([
    pool.query(revenueQuery, params),
    pool.query(closedQuery, params),
    pool.query(openQuery, params),
  ]);

  // KPI cards — current month, prior month, YTD, same month last year (Pacific time)
  const now = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Los_Angeles' }));
  const y = now.getFullYear();
  const m = now.getMonth(); // 0-based
  const currentYM = `${y}-${String(m + 1).padStart(2, '0')}`;
  const priorYM = m === 0 ? `${y - 1}-12` : `${y}-${String(m).padStart(2, '0')}`;
  const lastYearYM = `${y - 1}-${String(m + 1).padStart(2, '0')}`;
  const ytdStart = `${y}-01`;

  const kpiParams = [currentYM, priorYM, lastYearYM, ytdStart];
  const kpiProductFilter = filtered ? 'AND category = $5' : '';
  if (filtered) kpiParams.push(product);

  const { rows: kpiRows } = await pool.query(`
    SELECT
      ROUND(SUM(total_revenue) FILTER (WHERE fetch_month = $1)::numeric, 2) as mtd_rev,
      ROUND(SUM(total_revenue) FILTER (WHERE fetch_month = $2)::numeric, 2) as prior_rev,
      ROUND(SUM(total_revenue) FILTER (WHERE fetch_month = $3)::numeric, 2) as last_year_rev,
      ROUND(SUM(total_revenue) FILTER (WHERE fetch_month >= $4)::numeric, 2) as ytd_rev
    FROM order_summary
    WHERE file_number NOT ILIKE 'test%' AND file_number NOT ILIKE 'ar test%'
      ${kpiProductFilter}
  `, kpiParams);
  ...
  const k = kpiRows[0] || {};
  const mtdRev = parseFloat(k.mtd_rev) || 0;
  const priorRev = parseFloat(k.prior_rev) || 0;
  const lastYearRev = parseFloat(k.last_year_rev) || 0;
  const ytdRev = parseFloat(k.ytd_rev) || 0;

  return {
    revenue: pivotTrendRows(revenueRows),
    openOrders: pivotTrendRows(openRows),
    closedOrders: pivotTrendRows(closedRows),
    kpi: {
      mtdRevenue: mtdRev,
      priorRevenue: priorRev,
      priorChangePct: priorRev > 0 ? Math.round(((mtdRev - priorRev) / priorRev) * 100) : null,
      ytdRevenue: ytdRev,
      lastYearRevenue: lastYearRev,
      lastYearChangePct: lastYearRev > 0 ? Math.round(((mtdRev - lastYearRev) / lastYearRev) * 100) : null,
    },
