...
async function buildEscrowManagerEmailHtml(managerName) {
  ...
  const [{ rows: mtdRows }, { rows: ydayRows }, { rows: priorRows }, { rows: wd }, { rows: priorWorkDays }] = await Promise.all([
    pool.query(`
  ...
  const trend = pctChangePace(teamMtd, teamPrior, worked, priorTotalWorkingDays);
  ...
  <tr><td style="padding:4px 32px 0;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px; border-collapse:collapse;">
      ...
      <tr>
        ...
        <td colspan="2" align="right" style="padding:8px; color:#868e96; font-size:12px; font-weight:600;">${fmt(teamPrior)}${trend.pct !== null ? ` &nbsp;<span style="color:${trend.color};">${trend.pct >= 0 ? '\u25b2' : '\u25bc'} ${Math.abs(trend.pct)}%</span>` : ''}</td>
      </tr>
    </table>
  </td></tr>
