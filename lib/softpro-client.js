...
// ============================================
// OPEN ORDERS: Fetch from SoftPro API
// ============================================

/**
 * Fetch open orders from SoftPro API for a given date
 * Returns deduplicated orders (API returns line items, multiple per order)
 * @param {string} date - "YYYY-MM-DD" format (first of month)
 * @returns {Array} unique orders (deduplicated by file number)
 */
async function fetchOpenOrders(date) {
  ...
  try {
    ...
    // Deduplicate by order number — API returns multiple line items per order
    const orderMap = {};
    for (const record of rawData) {
      const orderNumber = g(record, 'Number');
      if (!orderNumber) continue;

      // Only keep the first occurrence — all line items share the same order metadata
      if (!orderMap[orderNumber]) {
        orderMap[orderNumber] = record;
      }
    }

    const uniqueOrders = Object.values(orderMap);
    console.log(`Open orders API returned ${rawData.length} line items → ${uniqueOrders.length} unique orders in ${duration}ms`);
 ...
 * @param {Array} records - deduplicated order records from fetchOpenOrders
 * @param {string} yearMonth - "YYYY-MM" format
 * @returns {object} { inserted, deleted, month }
 */
async function importOpenOrders(records, yearMonth) {
  ...
  try {
    ...
    for (const r of records) {
      ...
      const titleOfficer = (g(r, 'TitleOfficerName') || '').trim() || null;
      ...
      `, [
        fileNumber,
        parseDate(g(r, 'ReceivedDate')),
        parseDate(g(r, 'EscrowClosedDate')),  // settlement_date
        transType || null,
        orderType || null,
        (g(r, 'ProductType') || '').trim() || null,
        (g(r, 'OwnershipProfile') || '').trim() || null,
        branch,
        category,
