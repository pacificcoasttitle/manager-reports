    ...
    for (const order of orders) {
      await client.query(`
        INSERT INTO order_summary (
          ...
          fetch_month, line_item_count, marketing_source, main_contact, full_address,
          commissionable_escrow, officer_commissionable_escrow
      ...
      `, [
        ...
        yearMonth, order.line_item_count, order.marketing_source, order.main_contact, order.full_address,
        order.commissionable_escrow, order.officer_commissionable_escrow
      ]);
    }
 ...
 *   [EscrowOfficerName] → escrow_officer, [EscrowAssistant], [MarketingSource], [MainContact]
 *
  ...
  try {
    ...
    for (const r of records) {
      ...
      `, [
        ...
        titleOfficer,
        // Feed sends [EscrowOfficerName] (confirmed Jul 2026); the old 'EscrowOfficer'
        // key silently returned undefined -> null on every open order since Feb 2026.
        (g(r, 'EscrowOfficerName') || '').trim() || null,
    ...
    // GUARDRAIL: surface a field-mapping break early. If a month imports with
    // title_officer/sales_rep populated but escrow_officer is 0-populated, the
    // escrow-officer key almost certainly drifted (SoftPro drifts keys). This exact
    // break hid for 5 months (Feb–Jul 2026) because nothing watched for it.
    try {
      const { rows: cov } = await client.query(`
        SELECT COUNT(*) FILTER (WHERE escrow_officer IS NOT NULL AND escrow_officer != '') as esc,
               COUNT(*) FILTER (WHERE title_officer IS NOT NULL AND title_officer != '') as tit,
               COUNT(*) as total
        FROM open_orders WHERE open_month = $1`, [yearMonth]);
      const c = cov[0];
      if (parseInt(c.total) > 50 && parseInt(c.esc) === 0 && parseInt(c.tit) > 0) {
        console.warn(`GUARDRAIL: ${yearMonth} open orders have 0 escrow_officer but ${c.tit} title_officer (of ${c.total}) — likely a field-mapping break in the open-orders feed.`);
      }
    } catch (gErr) {
      console.error(`Open-orders coverage guardrail check failed for ${yearMonth}:`, gErr.message);
    }
