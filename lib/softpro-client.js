...
async function fetchMonth(yearMonth) {
  ...
  try {
    ...
    const filtered = rawRecords.filter(r => isValidBillCode(r['BillCode']));
    ...
    const lineItems = filtered.map(r => ({
      file_number: (r['Number'] || '').trim(),
      transaction_date: parseDate(r['TransactionDate']),
      bill_code: (r['BillCode'] || '').trim(),
      bill_code_category: r['BillCodeCategory'] || null,
      charge_description: r['ChargeDescription'] || null,
      sum_amount: parseFloat(r['SumAmount']) || 0,
      sales_rep: (r['SalesRep'] || '').trim() || null,
      title_officer: (r['TitleOfficerName'] || '').trim() || null,
      escrow_officer: (r['EscrowOfficerName'] || '').trim() || null,
      order_type: (r['OrderType'] || '').trim() || null,
      trans_type: (r['TransType'] || '').trim() || null,
      title_office: r['TitleOffice'] || null,
      escrow_office: r['EscrowOffice'] || null,
      property_type: r['PropertyType'] || null,
      county: r['County'] || null,
      city: r['City'] || null,
      state: r['PropState'] || null,
      zip: r['Zip'] || null,
      address: r['Address1'] || null,
      full_address: r['FullAddress'] || null,
      marketing_source: r['MarketingSource'] || null,
      main_contact: r['MainContact'] || null,
      underwriter: r['Underwriter'] || null,
      disbursement_date: parseDate(r['DisbursementDate']),
      escrow_closed_date: parseDate(r['EscrowClosedDate']),
      received_date: parseDate(r['ReceivedDate']),
