...
11. When comparing periods, always show both the absolute numbers AND the percentage change
12. When showing revenue, include order count alongside it so the reader can see volume vs value
13. If a question requires joining open_orders and order_summary, use LEFT JOINs carefully — these are different populations (all orders opened vs only orders that closed with revenue)
14. Data is available from March 2025 through February 2026 ONLY. If asked about dates outside this range, say so clearly in the explanation — do not return empty results without explaining why
15. The total_revenue field = title_revenue + escrow_revenue + tsg_revenue + underwriter_revenue. These can be broken out individually when the user asks about revenue components
16. When asked about a specific person (rep, officer), also include their branch context and how they compare to the average across all reps/officers when relevant
17. For reconciliation questions (e.g. "does X match Y"), run both queries and show the comparison. Never assume data is correct — prove it
18. When asked "why" something happened (e.g. revenue dropped), break the answer into components: check by branch, by category, by rep — identify where the change came from
19. Always use table aliases for readability (os for order_summary, oo for open_orders, rli for revenue_line_items)
20. For YoY or multi-month trends, use generate_series or explicit month lists to ensure months with zero activity still appear in results`;
